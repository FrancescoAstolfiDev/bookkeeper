...
def analyze_with_sonar(tag):
    ...
    # Crea un branch temporaneo
    temp_branch = f"temp-analysis-{tag}"
    run(f"git checkout -b {temp_branch}", cwd=REPO_PATH)
    ...
    time.sleep(30)  # Attendi 60 secondi

    # Torna indietro alla situazione originale
    run("git checkout master", cwd=REPO_PATH)
    run(f"git branch -D {temp_branch}", cwd=REPO_PATH)
